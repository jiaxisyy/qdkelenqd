...
public class ReadAndWrite {
    ...
    public static int WriteJni(int type, int[] str, String[] input) {
        switch (type) {
            case Constants.Define.OP_BIT_X:
                for (int i = 0; i < str.length; i++) {
            ...
            case Constants.Define.OP_WORD_D:
                ...
                for (int i = 0; i < str.length; i++) {
                    if (input[i] != null && !input[i].equals("")) {
                        short[] s = {Short.parseShort(input[i])};
                        int mdbuswriteword = MyApplication.getInstance().mdbuswriteword(Constants.Define.OP_WORD_D, s, str[i], 1);
                        return mdbuswriteword;
                    }
